document.addEventListener("DOMContentLoaded", () => {
 ...
 toastTitle.textContent = title;
  ...
  toastText.style.whiteSpace = "pre-wrap"; // 👈 CLAAAU
  ...
  function addPersona(prefillName = "") {
    ...
    // IMPORTANT: no fem servir style.display aquí. NOMÉS classes.
    // Neteja estat inicial (per si el template ha quedat "tocat")
    if (edatWrap) edatWrap.classList.remove("is-open");
    if (dietaAltresWrap) dietaAltresWrap.classList.remove("is-open");
...
showLocalizedToast();
function showLocalizedToast() {
  const lang = document.documentElement.lang;

  if (lang === "es") {
    showToast(
      "Respuesta enviada.\n\nLa organización declina cualquier responsabilidad por las consecuencias físicas, mentales o existenciales, incluyendo pero no limitándose a: resaca severa, deshidratación, agujetas, pérdida de voz, memoria selectiva o criterio dudoso 😈",
      "¡Perfecto!"
    );
  } else {
    showToast(
      "Resposta enviada.\n\nL'organització declina qualsevol responsabilitat per les conseqüències físiques, mentals o existencials, incloent però no limitant-se a: ressaca severa, deshidratació, agulletes, pèrdua de veu, memòria selectiva o criteri dubtós 😈",
      "Perfecte!"
    );
  }
}
